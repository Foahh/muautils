...
void ReplaceChunks(const std::span<const uint8_t> data, const fs::path &dstPath,
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        ...
        outFile.write(reinterpret_cast<const char *>(data.data() + cursor),
                      static_cast<std::streamsize>(s - cursor));
        if (replacements[i].has_value()) {
            auto &repl = replacements[i].value();
            outFile.write(reinterpret_cast<const char *>(repl.data()),
                          static_cast<std::streamsize>(repl.size()));
        } else {
            outFile.write(reinterpret_cast<const char *>(data.data() + s),
                          static_cast<std::streamsize>(e - s));
        }
        cursor = e;
    }
    outFile.write(reinterpret_cast<const char *>(data.data() + cursor),
