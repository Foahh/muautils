...
std::optional<size_t> FindChunks(std::span<const uint8_t> haystack,
                                  std::span<const uint8_t> needle,
                                  size_t start);

std::vector<std::pair<size_t, size_t>> LocateChunks(std::span<const uint8_t> data,
                                                     std::span<const uint8_t> header,
                                                     std::span<const uint8_t> footer);

std::vector<std::pair<size_t, size_t>> LocateDdsChunks(std::span<const uint8_t> data);

void ExtractChunks(std::span<const uint8_t> data, const fs::path &dstFolder,
