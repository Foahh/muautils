...
void pumpDecoder(const AVCodecContextPtr &decoder,
    ...
    for (;;) {
        ...
        av::Assert(ret, "Failed to add frame to buffer source: {}", src->filter->name);
...
void RunGraph(const AVFormatInputContextPtr &input,
    ...
    while (av_read_frame(input.get(), pkt.get()) >= 0) {
        ...
        av::Assert(ret, "Failed to send packet to decoder: {}", decoder->codec->name);
        av_packet_unref(pkt.get());
        pumpDecoder(decoder, src, sink, dfrm, ffrm, onFrame);
    }

    auto ret = avcodec_send_packet(decoder.get(), nullptr);
    av::Assert(ret, "Failed to send end-of-stream packet to decoder: {}", decoder->codec->name);
    pumpDecoder(decoder, src, sink, dfrm, ffrm, onFrame);

    ret = av_buffersrc_add_frame(src, nullptr);
    av::Assert(ret, "Failed to add end-of-stream frame to buffer source: {}", src->filter->name);
...
void Encode(const AVFramePtr &frame,
    ...
    av::Assert(ret, "Failed to send frame to encoder: {}", encoder->codec->name);

    while (avcodec_receive_packet(encoder.get(), pkt.get()) == 0) {
        ret = av_interleaved_write_frame(output.get(), pkt.get());
        av::Assert(ret, "Failed to write packet to output format: {}", output->oformat->name);
        av_packet_unref(pkt.get());
    }
}
