...
$OverlayDir = Join-Path $ScriptDir 'openimageio-port-overlay'
$SrcPort = 'ports/openimageio'
...
function Show-Usage
{
    ...
    Write-Host ''
    Write-Host "  Clean ports/openimageio, copy $SrcPort from the vcpkg checkout, then apply"
    Write-Host "  patches from $OverlayDir/ (see muautils-openimageio.patch)."
    Write-Host ''
